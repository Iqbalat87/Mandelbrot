...
def is_in_madnelbrot(c, r=5, nb_ite=20):
    z = c
    if c.mod == 0:
        return True
    ...
    if abs(c.re - z.re) <= r and abs(c.im - z.im) <= r:
        return (abs(c.re - z.re) + abs(c.im - z.im)) / 2

    return 0

def calc_color(speed, ratio=255/2):
    r = abs(speed * ratio - 1/speed)
    if r >= 255:
        r = 255

    return int(r)

def create_image(w, h, zoom=1, center=(0,0), precision=(5, 20)):
    img = Image.new("RGB", (w, h), (255, 255, 255))
    re_p = zoom
    im_p = zoom
    for i in range(w):
        for j in range(h):
            c = Complexe(re = center[0] + re_p * (i - w/2), im = center[1] + im_p * (j - h/2))
            speed = is_in_madnelbrot(c, precision[0], precision[1])
            if speed != 0:
                color = (calc_color(speed), 0, 0)
...
center = (0.457, 0.3715)
zoom = 0.00001
img = create_image(600, 500, zoom, center, (15, 10))
img.show()
img.save("Mandelbrot_{}_{}.png".format(zoom, center))
